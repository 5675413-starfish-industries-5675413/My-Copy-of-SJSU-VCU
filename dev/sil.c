...
// If not, uncomment one of the following based on your build setup:
// #include "sil/build_main_sil/parse_values.h"
// #include "../sil/build_main_sil/parse_values.h"
...
void sil_write_json_output(MotorController* mcm, PowerLimit* pl, Efficiency* eff)
{
    ...
    if ((sil_output_mode & SIL_OUTPUT_EFFICIENCY) && eff != NULL) {
        if (!first_field) printf(",");
        printf("\"efficiency\":{");
        int first_eff = 1;
        
        if (mcm != NULL) {
            float4 mcm_motorRPM = MCM_getMotorRPM(mcm);
            if (!first_eff) printf(",");
            printf("\"mcm_motorRPM\":%.4f", mcm_motorRPM);
            first_eff = 0;
        }
